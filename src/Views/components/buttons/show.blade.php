...
<span>
    ...
    <tooltip :on-click=true
        ...
        <div class="flex flex-col gap-1">
            ...
            {{-- Context --}}
            <a class="btn-nav small btn-success" href="{{ action([$controller_context, 'show'], [$item->getKey()]) }}">
                {!! Template::icon('list') . ' ' . ucfirst(trans('imet-core::common.context')) !!}
            </a>

            {{-- Evaluation --}}
            <a class="btn-nav small btn-success" href="{{ action([$controller_eval, 'show'], [$item->getKey()]) }}">
            ...
            @if($version===\AndreaMarelli\ImetCore\Models\Imet\Imet::IMET_V2 || $version===\AndreaMarelli\ImetCore\Models\Imet\Imet::IMET_OECM)
                <a class="btn-nav small btn-success" href="{{ action([$controller_report, 'report_show'], [$item->getKey()]) }}">

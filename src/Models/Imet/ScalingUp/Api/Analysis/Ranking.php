...
trait Ranking
{
    ...
    private static function retrieve_data_ranking(array $items, array $indicators, string $type = 'context'): array
    {
        ...
        foreach ($ranking['xAxis'] as $key => $xAxi) {
            $api[] = ['id' => $ranking['wdpa_ids'][$key], 'name' => $xAxi, 'values' => []];
        }

        foreach ($ranking['values'] as $key => $value) {
            foreach ($value as $k => $v) {
                $indicator = array_keys($ranking['legends'], $key)[0];
